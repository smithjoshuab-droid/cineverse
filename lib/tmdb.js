...
const CACHE_TTL_MS = 10 * 60 * 1000;      // lists, search
const DAY_TTL_MS = 24 * 60 * 60 * 1000;   // providers, external ids, genres, tv basics
...
async function tmdb(pathname, params = {}, ttl = CACHE_TTL_MS) {
  ...
  if (hit && Date.now() - hit.at < (hit.ttl || CACHE_TTL_MS)) return hit.data;
  ...
  cache.set(cacheKey, { at: Date.now(), ttl, data });
  if (cache.size > 500) {
...
async function getGenres(type) {
  const data = await tmdb(`/genre/${type}/list`, {}, DAY_TTL_MS);
  return data.genres || [];
}
...
async function imdbIdFor(type, id) {
  try {
    const data = await tmdb(`/${type}/${id}/external_ids`, {}, DAY_TTL_MS);
...
async function tvBasics(id) {
  try {
    const d = await tmdb(`/tv/${id}`, {}, DAY_TTL_MS);
...
async function providers(type, id) {
  try {
    const prov = await tmdb(`/${type}/${id}/watch/providers`, {}, DAY_TTL_MS);
    const regional = (prov.results || {})[REGION] || {};
    const flat = [...(regional.flatrate || []), ...(regional.ads || []), ...(regional.free || [])];
